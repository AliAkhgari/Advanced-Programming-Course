...
void POST::read_command()
{
    ...
    else if(command == DEFAULT_PRICE_FILTER_COMMAND)
    {
        string active;
        try {
            active = find_value(ACTIVE);
            utrip->default_price_filter(active);
            cout << OK;
        } catch(const exception& e) {
            cout << e.what() << '\n';
        }
    ...
    else if(command == SORT_COMMAND)
    {
        ...
        try {
            property = find_value(PROPERTY);
            order = find_value(ORDER);
            utrip->sort_hotels(property, order);
            cout << OK;
    ...
    else if(command == MANUAL_WEIGHTS_COMMAND)
    {
        ...
        try {
            ...
            if(active == "true")
            {
                double location = stod(find_value(LOCATION)), cleanliness = stod(find_value(CLEANLINESS)),
                       staff = stod(find_value(STAFF)), facilities = stod(find_value(FACILITIES)),
                       value_for_money = stod(find_value(VALUE_FOR_MONEY));
                utrip->set_manual_weights(location, cleanliness, staff, facilities, value_for_money);
                cout << OK;
            }
            else if(active == "false")
            {
                utrip->deactive_manual_weights();
                cout << OK;
            }
            else
                throw ExceptionHandling(BAD_REQUEST_ERROR);
